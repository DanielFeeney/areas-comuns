...
export default function ListReserva(){
  ...
  async function fetchDataAprovadas(){
    const response = await fetch('http://localhost:8080/area-comum/v1/reserva/aprovadas')
  ...
  const cancelar = async (id) => {
    ...
    const response = await fetch(`http://localhost:8080/area-comum/v1/reserva/cancelar/${id}`, requestOptions);
    const data = await response.json();
    await fetchDataAprovadas();
  }
        ...
        <TableBody>
          {areas.map((area) => (
            <TableRow key={area.id}>
              ...
              <TableCell align="right"><Button variant="contained" color="primary" onClick={() => cancelar(area.id)}>Cancelar</Button></TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
