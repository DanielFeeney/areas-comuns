...
export default function ListReserva(){
  ...
  async function fetchDataCancelada(){
    const response = await fetch('http://localhost:8080/area-comum/v1/reserva/canceladas')
        ...
        <TableBody>
          {areas.map((area) => (
            <TableRow key={area.id}>
              ...
              <TableCell align="right"><Button variant="contained" disabled color="primary">Cancelada</Button></TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
